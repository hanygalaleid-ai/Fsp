...
namespace Fsp.BattleRoyale
{
    public sealed class SafeZoneController : MonoBehaviour
    {
        ...
        private void TryInitialize(bool force = false)
        {
            ...
            initialized = true;
            PhaseIndex = -1;
            initialCenter = Flatten(transform.position);
            ...
            RefreshVisual();
            if (!authoritativeClock) StartNextPhase();
        }
        ...
        private void Update()
        {
            ...
            {
                float t = 1f - StateTimeRemaining / stateDuration;
                float eased = Mathf.SmoothStep(0f, 1f, t);
                Center = Vector3.Lerp(phaseStartCenter, TargetCenter, eased);
                CurrentRadius = Mathf.Lerp(phaseStartRadius, TargetRadius, eased);
                ApplyTransformAndNotify();
            }
            if (StateTimeRemaining > 0f) return;
            if (State == PhaseState.Waiting) BeginShrink();
            else { Center = TargetCenter; CurrentRadius = TargetRadius; ApplyTransformAndNotify(); StartNextPhase(); }
        }
        ...
        public void ApplyAuthoritativeElapsed(float elapsedSeconds)
        {
            ...
            Vector3 center = initialCenter;
            float radius = plan.initialRadius;
            int phaseCount = plan.phases != null ? plan.phases.Length : 0;

            for (int i = 0; i < phaseCount; i++)
            {
                SafeZonePlan.Phase phase = plan.phases[i];
                float nextRadius = Mathf.Max(12f, radius * Mathf.Clamp(phase.radiusFactor, 0.05f, 1f));
                Vector3 nextCenter = PickNextCenter(center, radius, nextRadius, phase.centerShiftFactor, i);
                float wait = Mathf.Max(0f, phase.waitSeconds);
                float shrink = Mathf.Max(1f, phase.shrinkSeconds);

                if (remaining < wait)
                {
                    SetAuthoritativeState(i, PhaseState.Waiting, center, radius, nextCenter, nextRadius, wait - remaining, phase.damagePerSecond);
                    return;
                }
                remaining -= wait;
                ...
                if (remaining < shrink)
                {
                    float t = Mathf.Clamp01(remaining / shrink);
                    float eased = Mathf.SmoothStep(0f, 1f, t);
                    SetAuthoritativeState(i, PhaseState.Shrinking, Vector3.Lerp(center, nextCenter, eased), Mathf.Lerp(radius, nextRadius, eased), nextCenter, nextRadius, shrink - remaining, phase.damagePerSecond);
                    return;
                }
            ...
            CurrentDamagePerSecond = phaseCount > 0 ? plan.phases[phaseCount - 1].damagePerSecond : 0f;
            ApplyTransformAndNotify();
        }

        private void SetAuthoritativeState(int index, PhaseState state, Vector3 center, float radius, Vector3 targetCenter, float targetRadius, float remaining, float damage)
        {
            bool phaseChanged = PhaseIndex != index || State != state;
            PhaseIndex = index;
            State = state;
            Center = center;
            CurrentRadius = radius;
            ...
            StateTimeRemaining = Mathf.Max(0f, remaining);
            CurrentDamagePerSecond = damage;
            ApplyTransformAndNotify();
            if (phaseChanged) PhaseChanged?.Invoke(PhaseIndex, State, StateTimeRemaining);
        }
        ...
        private void StartNextPhase()
        {
            ...
            SafeZonePlan.Phase phase = plan.phases[PhaseIndex];
            phaseStartCenter = Center;
            phaseStartRadius = CurrentRadius;
            TargetRadius = Mathf.Max(12f, CurrentRadius * Mathf.Clamp(phase.radiusFactor, 0.05f, 1f));
            TargetCenter = PickNextCenter(Center, CurrentRadius, TargetRadius, phase.centerShiftFactor, PhaseIndex);
            CurrentDamagePerSecond = phase.damagePerSecond;
            State = PhaseState.Waiting;
            stateDuration = Mathf.Max(0f, phase.waitSeconds);
            StateTimeRemaining = stateDuration;
            PhaseChanged?.Invoke(PhaseIndex, State, StateTimeRemaining);
            ZoneChanged?.Invoke();
        }
        ...
        private void BeginShrink()
        {
            ...
            phaseStartCenter = Center;
            phaseStartRadius = CurrentRadius;
            State = PhaseState.Shrinking;
            stateDuration = Mathf.Max(1f, phase.shrinkSeconds);
            StateTimeRemaining = stateDuration;
            PhaseChanged?.Invoke(PhaseIndex, State, StateTimeRemaining);
        }
        ...
        private void ApplyTransformAndNotify()
        {
