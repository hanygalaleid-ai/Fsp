...
namespace Fsp.BattleRoyale
{
    public sealed class SafeZoneController : MonoBehaviour
    {
        ...
        {
            Vector2 delta = new(worldPosition.x - Center.x, worldPosition.z - Center.z);
            return delta.sqrMagnitude <= CurrentRadius * CurrentRadius;
        }

        public float OutsideDamagePerSecond(Vector3 worldPosition)
            => IsInside(worldPosition) ? 0f : CurrentDamagePerSecond;

        private void StartNextPhase()
        {
            PhaseIndex++;
            if (plan.phases == null || PhaseIndex >= plan.phases.Length)
            {
                State = PhaseState.Complete;
                StateTimeRemaining = 0f;
                PhaseChanged?.Invoke(PhaseIndex, State, 0f);
                return;
            }
