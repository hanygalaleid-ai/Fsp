...
public sealed class StarterSpawnBalance:MonoBehaviour{
 ...
 void Start(){var spawner=Object.FindObjectOfType<BotSpawner>();if(spawner==null)return;var root=new GameObject("BalancedBotSpawns").transform;root.SetParent(transform,false);var points=new Transform[Anchors.Length];for(int i=0;i<Anchors.Length;i++){var g=new GameObject("BotSpawn_"+i);g.transform.SetParent(root,false);g.transform.position=Anchors[i];points[i]=g.transform;}spawner.ConfigureSpawnPoints(points);}
}}
