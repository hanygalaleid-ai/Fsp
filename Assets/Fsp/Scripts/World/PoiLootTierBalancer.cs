...
public sealed class PoiLootTierBalancer:MonoBehaviour{
 ...
 void Start(){Balance();}
 public void Balance(){var pickups=UnityEngine.Object.FindObjectsByType<LootPickup>(FindObjectsSortMode.None);foreach(var pickup in pickups){if(pickup==null)continue;var tier=FindTier(pickup.transform.position);if(!tier.HasValue)continue;ApplyTier(pickup,tier.Value);}}
