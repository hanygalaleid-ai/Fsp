...
namespace Fsp.EditorTools
{
    ...
    public static class FspProjectBootstrap
    {
        ...
        private const string PrefKey = "Fsp.ProjectBootstrap.v4";
        ...
        {
            // Batch-mode startup already refreshes generated scenes once. During BuildPipeline
            // preprocessing we must not recreate scenes again because changing/opening scenes while
            // a player build is in progress can make the build unstable. Only bootstrap here when
            // the generated scenes are genuinely missing, otherwise just restore build settings.
        ...
        private static void EnsureProject()
        {
            ...
            try
            {
                ...
                // Scenes generated by older bootstrap versions were placeholders. Rebuild them in
                // batch/cloud mode once during editor startup so every APK contains the full setup.
                if (Application.isBatchMode || !File.Exists(LobbyScene)) CreateLobbyScene(true);
                if (Application.isBatchMode || !File.Exists(MatchScene)) CreateMatchScene(true);
                ...
                Debug.Log($"Fsp build bootstrap ready: {scenes.Length} scene(s) configured; full Lobby runtime UI enabled; Lobby={File.Exists(LobbyScene)}, Match={File.Exists(MatchScene)}.");

                if (!EditorPrefs.GetBool(PrefKey, false))
                {
                    EditorPrefs.SetBool(PrefKey, true);
                    Debug.Log("Fsp project initialized with complete generated Lobby and Match scenes.");
                }
            }
            finally
            {
        ...
        private static void CreateLobbyScene(bool overwrite)
        {
            ...
            stage.transform.position = new Vector3(-1.25f, 0f, 0f);
            stage.transform.localScale = new Vector3(1.8f, 0.10f, 1.8f);
            var renderer = stage.GetComponent<Renderer>();
            if (renderer != null) renderer.sharedMaterial.color = new Color(0.22f, 0.19f, 0.15f);
