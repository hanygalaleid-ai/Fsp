...
const CLIENT_TYPES = new Set(["snapshot", "bot_snapshot", "fire", "damage", "vehicle", "seat", "loot_claim", "appearance"]);
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    const headers = new Headers({ Upgrade: "websocket", "x-fsp-player-id": userId });
...
export class MatchRoom extends DurableObject<Env> {
  ...
  async webSocketMessage(ws: WebSocket, message: string | ArrayBuffer): Promise<void> {
    ...
    if (envelope.type === "bot_snapshot") {
      await this.handleBotSnapshot(ws, attachment.playerId, payload);
      return;
    }
    ...
    if (envelope.type === "vehicle") {
      ...
      if (!vehicleId) return;
      if ((await this.ctx.storage.get<string>(`seat:${vehicleId}`)) !== attachment.playerId) return;
    ...
    if (envelope.type === "seat") {
      await this.handleSeat(attachment.playerId, payload);
      return;
    }

    if (envelope.type === "loot_claim") {
      await this.handleLoot(attachment.playerId, payload);
    }
  }
  ...
  private async handleSnapshot(ws: WebSocket, playerId: string, payload: Record<string, unknown>, originalMessage: string): Promise<void> {
    ...
    if (previous?.alive && !alive) {
      ...
      this.broadcast(JSON.stringify({
        type: "elimination",
        payload: JSON.stringify({ killerId, victimId: playerId, timestamp: now / 1000 })
      }));
  ...
  private async handleSeat(playerId: string, payload: Record<string, unknown>): Promise<void> {
    ...
    if (seated) {
      accepted = existing == null || existing === playerId;
      if (accepted) await this.ctx.storage.put(key, playerId);
    } else {
      accepted = existing == null || existing === playerId;
      if (existing === playerId) await this.ctx.storage.delete(key);
    }

    this.broadcast(JSON.stringify({
      type: "seat",
      payload: JSON.stringify({ playerId, vehicleId, seated, accepted, timestamp: Date.now() / 1000 })
    }));
  }
  ...
  private async handleLoot(playerId: string, payload: Record<string, unknown>): Promise<void> {
    ...
    this.broadcast(JSON.stringify({
      type: "loot_claimed",
      payload: JSON.stringify({ playerId: accepted ? playerId : existing, lootId, accepted, timestamp: Date.now() / 1000 })
    }));
  }
  ...
  private async ensureBotAuthority(preferredPlayerId?: string): Promise<void> {
    ...
    if (current && this.isPlayerConnected(current)) {
      this.broadcastBotAuthority(current);
      return;
    }
  ...
  private broadcastBotAuthority(playerId: string): void {
    this.broadcast(JSON.stringify({
      type: "bot_authority",
      payload: JSON.stringify({ playerId, timestamp: Date.now() / 1000 })
    }));
  }
  ...
  private async broadcastMatchState(): Promise<void> {
    ...
    this.broadcast(JSON.stringify({
      type: "match_state",
      payload: JSON.stringify({ aliveCount: aliveIds.length, totalCount: states.size, winnerId, finished: !!winnerId, timestamp: Date.now() / 1000 })
    }));
  }
  ...
  async webSocketClose(ws: WebSocket, code: number, reason: string): Promise<void> {
    ...
    if (attachment?.playerId) {
      ...
      const authority = await this.ctx.storage.get<string>("bot-authority");
      if (authority === attachment.playerId) {
        await this.ctx.storage.delete("bot-authority");
        await this.ensureBotAuthority();
      }
    }
...
function normalize(v: Vec3): Vec3 | null {
  ...
  return Number.isFinite(m) && m >= 0.0001 ? { x: v.x / m, y: v.y / m, z: v.z / m } : null;
}
